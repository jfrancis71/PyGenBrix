...
#        torch.nn.Conv2d( 1, 1, 1), nn.Tanh(),
        torch.nn.Conv2d( 2, dims[0]*1, 3, padding=1 ).to( device )
...
class ParallelCNN( nn.Module ):
    ...
    def log_prob( self, samples, conditional_input = None ):
        ...
        i0 = torch.cat( ( samples*torch.tensor( self.information_masks[0].astype( np.float32 ) ).to( self.device ), self.position_layer.expand_as( samples ) ), dim=1 )
...
#        log0 = i0[:,1:2]
        ...
        i1 = torch.cat( ( samples*torch.tensor( self.information_masks[1].astype( np.float32 ) ).to( self.device ), self.position_layer.expand_as( samples ) ), dim=1 )
        ...
        i2 = torch.cat( ( samples*torch.tensor( self.information_masks[2].astype( np.float32 ) ).to( self.device ), self.position_layer.expand_as( samples ) ), dim=1 )
        ...
        i3 = torch.cat( ( samples*torch.tensor( self.information_masks[3].astype( np.float32 ) ).to( self.device ), self.position_layer.expand_as( samples ) ), dim=1 )
